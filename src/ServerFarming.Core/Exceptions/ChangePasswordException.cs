using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace ServerFarming.Core.Exceptions
{
    /// <summary>
    /// Exception when user change password FAIL by some errors
    /// </summary>
    public class ChangePasswordException : Exception
    {
        public IEnumerable<string> ErrorMessages { get; set; }

        public ChangePasswordException(string message, Exception innerException = null)
            : base(message, innerException)
        {
            this.ErrorMessages = new[] { message };
        }

        public ChangePasswordException(IEnumerable<string> messages, Exception innerException = null)
            : base(string.Join("\r\n", messages), innerException)
        {
            ErrorMessages = messages;
        }
    }
}
